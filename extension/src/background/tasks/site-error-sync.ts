  ...
  try {
    ...
    for (let i = 0; i < malls.length; i++) {
      ...
      try {
            ...
            if (item.skuList && Array.isArray(item.skuList)) {
              for (const sku of item.skuList) {
                if (sku.goodsSkuId && sku.skcId) {
                  skuMapping.set(String(sku.goodsSkuId), sku.skcId)
                }
              }
            }
          }
        ...
        for (
          let batchStart = 0;
          batchStart < goodsIdArray.length;
          ...
          try {
            ...
            // 步骤 2.4: 解析站点异常
            if (errorResult.result?.fullyBindSiteFailVO) {
              for (const skcError of errorResult.result.fullyBindSiteFailVO) {
